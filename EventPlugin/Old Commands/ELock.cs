...
namespace ATTG3
{
    class ELock : ICommandHandler
    {
        private readonly ATTG3Plugin plugin;
        Server Server => PluginManager.Manager.Server;
        IConfigFile Config => ConfigManager.Manager.Config;
        public ELock(ATTG3Plugin plugin) => this.plugin=plugin;
        ...
        public string[] OnCall(ICommandSender sender, string[] args)
        {
            ...
            else
            {
                ...
                if (args.Length>0)
                {
                    ...
                    {
                        plugin.ELockdownact=!plugin.ELockdownact;
                        plugin.EULockdownact = false;
                        plugin.EPlayerUD=null;
                        if (plugin.ELockdownact)
                        {
                            plugin.EPlayerLD=myPlayer.SteamId;
                ...
                {
                    return new string[] { "AGELOCK: "+GetUsage() };
                }
            }
        }
    }
