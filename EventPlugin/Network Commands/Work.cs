            ...
            if (args.Length>0)
            {
                ...
                if (myPlayer.TeamRole.Role!=Role.SPECTATOR)
                {
					...
					GameObject val = GameObject.Instantiate(Work1, player1.transform.position, Quaternion.identity);
                        val.GetComponent<WorkStation>().Networkposition=new Offset {position=new Vector3(pos.x, pos.y, pos.z+2),rotation=player1.transform.localPosition, scale=Vector3.one};
